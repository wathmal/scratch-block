...
//	private static const font13:Array = ['ar'];
	...
	public static function initializeLanguageList():void {
		...
		languages = WireMe.app.server.getLanguageList();
	...
	public static function setLanguage(lang:String):void {
		...
		setFontsFor(lang);
		if ('en' == lang){
			WireMe.app.translationChanged(); // there is no .po file English
		}else {
			var data:Object = WireMe.app.server.getPOFile(lang);
			if (data) {
				dictionary = data;
				checkBlockTranslations();
				setFontsFor(lang);
				WireMe.app.extensionManager.parseAllTranslators();
			}
			setTimeout(WireMe.app.translationChanged, 0);
//			WireMe.app.translationChanged();
		}
		langChangedSignal.dispatchEvent(new Event(Event.CHANGE));

		WireMe.app.server.setSelectedLang(lang);
	}
	...
	private static function fontSizeMenu():void {
		...
		m.showOnStage(WireMe.app.stage);
	}
...
//		}
	...
	private static function checkBlockSpec(spec:String):void {
		...
		if (!argsMatch(extractArgs(spec), extractArgs(translatedSpec))) {
			WireMe.app.log('Block argument mismatch:');
			WireMe.app.log('    ' + spec);
			WireMe.app.log('    ' + translatedSpec);
